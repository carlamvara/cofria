...
import { ShieldCheck, Mail, CheckCircle, Sparkles } from 'lucide-react';
...
export const CustodianView: React.FC<CustodianViewProps> = ({ t, navigate }) => {
  ...
  const CustodianTermsModal = () => (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
      <div className="bg-white p-8 md:p-12 max-w-lg w-full shadow-2xl rounded-sm border border-[#E5E0D6] animate-fade-in-up">
        <h3 className={`${theme.fontSerif} text-2xl mb-6`}>{t.custodian.readTerms}</h3>
        <p className="text-sm text-gray-600 mb-4">{t.custodian.termsIntro}</p>
        <ul className="list-disc pl-5 space-y-3 mb-8 text-sm text-gray-600">
          {t.custodian.termsList.map((term: string, i: number) => (
            <li key={i}>{term}</li>
          ))}
        </ul>
        <Button onClick={() => setShowTerms(false)} className="w-full">{t.custodian.termsBtn}</Button>
      </div>
    </div>
  );
